...
const std::vector<std::vector<double>> coordsVector = {
...
TEST(concave_iou, convertVectorToCuda) {
    for (int i = 0; i < coordsVector.size(); ++i) {
        for (int j = 0; j < coordsVector[j].size(); ++j)
            cudaArray[i * MAX_J + j] = coordsVector[i][j];
        cudaNumber[i] = coordsVector[i].size();
    }
    // force copy host data to device
    // https://stackoverflow.com/questions/34041372/access-cuda-global-device-variable-from-host
    cudaDeviceSynchronize();
}

TEST(concave_iou, delaunator) {
    for (auto &v : coordsVector) {
        validate(v);
    }
}

// TODO: force this function only run on device
CUDA_TEST(concave_iou, Delaunator) {
    ...
    while (cudaNumber[index] > 0) {
        ...
        for (std::size_t i = 0; i < d.halfedges_size; ++i) {
            const auto i2 = d.halfedges[i];
            ASSERT_EQ(!static_cast<bool>((i2 != INVALID_INDEX) &&
                                         (d.halfedges[i2] != i)),
                      true);
        }
        index++;
